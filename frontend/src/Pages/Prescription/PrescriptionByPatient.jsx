  ...
  return (
    <div className="mx-auto p-4 sm:ml-64  mt-16 background_patient">
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse mt-10  border border-gray-500">
          <thead>
            <tr>
              <th className="border-b  p-2">Prescription</th>
              <th className="border-b  p-2">Date</th>
              <th className="border-b  p-2">View</th>
          ...
          <tbody className='text-center'>
            {prescription.length > 0 ? (
              prescription.map((item) => (
                <tr key={item._id}>
                  <td className="border-b  p-2">{item.prescription}</td>
                  <td className="border-b  p-2">{new Date(item.Date).toLocaleDateString()}</td>
                  <td className="border-b  p-2">
                    <NavLink to={`/viewPrescription/${item._id}`}>
                      <button className="bg-transparent border-2 border-cyan-400 hover:bg-cyan-400 text-cyan-400 hover:text-white  font-bold py-2 px-4 rounded">View</button>
                    </NavLink>
                  </td>
                  <td className="border-b  p-2">
